...
class TodoListView {
    ...
    buildListItem(listItem, listItemIndex) {
        ...
        let upArrowDiv = document.createElement(TodoHTML.DIV);
        upArrowDiv.setAttribute(TodoHTML.ID, TodoGUIId.ITEM_CARD_ + listItemIndex + TodoGUIId.ITEM_CARD_MOVEUP);
        upArrowDiv.setAttribute(TodoHTML.CLASS, TodoGUIClass.LIST_ITEM_CARD_BUTTON);
        let upArrowImage = document.createElement("img");
        upArrowImage.setAttribute("src", "images/icons/MoveUp.png");
        upArrowDiv.appendChild(upArrowImage);
        itemArgs = [listItemIndex];
        if (listItemIndex != 0){
            this.setupCallback(upArrowDiv, TodoHTML.ONCLICK, TodoCallback.PROCESS_MOVE_ITEM_UP, itemArgs);
            upArrowDiv.setAttribute("style", "background-color: #669966;");
        }
        else{
            this.setupCallback(upArrowDiv, TodoHTML.ONCLICK, TodoCallback.PROCESS_DO_NOTHING, itemArgs);
            upArrowDiv.setAttribute("style", "background-color: #E6E6E6;");
        }

        let downArrowDiv = document.createElement(TodoHTML.DIV);
        downArrowDiv.setAttribute(TodoHTML.ID, TodoGUIId.ITEM_CARD_ + listItemIndex + TodoGUIId.ITEM_CARD_MOVEDOWN);
        downArrowDiv.setAttribute(TodoHTML.CLASS, TodoGUIClass.LIST_ITEM_CARD_BUTTON);
        let downArrowImage = document.createElement("img");
        downArrowImage.setAttribute("src", "images/icons/MoveDown.png");
        downArrowDiv.appendChild(downArrowImage);
        itemArgs = [listItemIndex];
        if (listItemIndex != window.todo.model.getNumItems()-1){
            this.setupCallback(downArrowDiv, TodoHTML.ONCLICK, TodoCallback.PROCESS_MOVE_ITEM_DOWN, itemArgs);
            downArrowDiv.setAttribute("style", "background-color: #669966;");
        }
        else{
            this.setupCallback(downArrowDiv, TodoHTML.ONCLICK, TodoCallback.PROCESS_DO_NOTHING, itemArgs);
            downArrowDiv.setAttribute("style", "background-color: #E6E6E6;");
        }
    
        let deleteItemDiv = document.createElement(TodoHTML.DIV);
        deleteItemDiv.setAttribute(TodoHTML.ID, TodoGUIId.ITEM_CARD_ + listItemIndex + TodoGUIId.ITEM_CARD_DELETE);
        deleteItemDiv.setAttribute(TodoHTML.CLASS, TodoGUIClass.LIST_ITEM_CARD_BUTTON);
        let deleteItemImage = document.createElement("img");
        deleteItemImage.setAttribute("src", "images/icons/Close.png");
        deleteItemDiv.appendChild(deleteItemImage);
        itemArgs = [listItemIndex];
        this.setupCallback(deleteItemDiv, TodoHTML.ONCLICK, TodoCallback.PROCESS_DELETE_ITEM, itemArgs);
        deleteItemDiv.setAttribute("style", "background-color: #669966;");
        ...
        toolbarDiv.appendChild(upArrowDiv);
        toolbarDiv.appendChild(downArrowDiv);
        toolbarDiv.appendChild(deleteItemDiv);
        newItemDiv.appendChild(toolbarDiv);

        return newItemDiv;
    }
    buildHeading(){
        let listHeadingDiv = document.createElement(TodoHTML.DIV);
        listHeadingDiv.setAttribute(TodoHTML.ID, TodoGUIId.LIST_HEADING);
        let trashDiv = document.createElement(TodoHTML.DIV);
        trashDiv.setAttribute(TodoHTML.ID, TodoGUIId.LIST_TRASH);
        let trashImage = document.createElement("img");
        trashImage.setAttribute("src", "images/icons/delete_black_24x24.png");
        trashDiv.appendChild(trashImage);
        this.setupCallback(trashDiv, TodoHTML.ONCLICK, TodoCallback.PROCESS_DELETE_LIST, callbackArguments);

        listHeadingDiv.appendChild(trashDiv);
        return listHeadingDiv;
    ...
    buildListItemsHeader() {
        ...
        // Added sort by due date functionality
